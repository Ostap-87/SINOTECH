#!/usr/bin/env bash
# СУПЕРСЕДЕН (2026-09-06) — используйте scripts/fix-soft-404-and-www.sh вместо
# этого файла. Этот скрипт когда-то уже применялся (см. историю ниже), но
# его результат был молча затёрт последующим повторным запуском
# deploy-vps.sh, который пишет тот же файл конфига без www-редиректа —
# отсюда и регрессия, обнаруженная 06.09.2026. Новый скрипт объединяет этот
# фикс с фиксом soft-404 и оставляет заметку в deploy-vps.sh, чтобы это не
# повторилось снова. Файл оставлен для истории, не удалять.
#
